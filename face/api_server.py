...
import torch
import numpy as np
from fastapi import FastAPI, File, UploadFile, WebSocket, WebSocketDisconnect, Header
...
import uuid # 用于生成用户ID和TOKEN
import base64 # Added for image encoding/decoding
...
action_model_path = str(MODEL_PATHS['action_detection'])
...
EMOTION_DISPLAY_THRESHOLD = RECOGNITION_THRESHOLDS['emotion_display_confidence']
...
ACTION_DETECTION_THRESHOLD = RECOGNITION_THRESHOLDS['action_detection_confidence']
ACTION_DISPLAY_THRESHOLD = RECOGNITION_THRESHOLDS['action_display_confidence']
...
print(f"📥 加载行为识别模型: {action_model_path}")
...
def match_face(features, database, threshold=None):
    ...
    # 如果最佳相似度低于阈值，返回Unknown
    if best_similarity < threshold:
        print(f"❌ 最佳相似度 {best_similarity:.6f} 低于阈值 {threshold}，返回Unknown")
        return "Unknown", best_similarity
    else:
        print(f"✅ 最佳相似度 {best_similarity:.6f} 达到阈值 {threshold}，返回 {best_match}")
...
def recognize_emotion(face_crop, emotion_model):
    ...
    try:
        print(f"=== 情绪识别开始 ===")
        print(f"输入图像形状: {face_crop.shape}")
        
        results = emotion_model(face_crop)
        print(f"模型结果数量: {len(results)}")
        ...
        if len(results) > 0:
            ...
            if hasattr(result, 'boxes') and result.boxes is not None and len(result.boxes) > 0:
                print(f"检测到 {len(result.boxes)} 个检测框")
                ...
                if hasattr(result.boxes, 'cls') and result.boxes.cls is not None:
                    ...
                    if class_id < len(emotion_labels):
                        ...
                        print(f"原始情绪标签: {emotion}")
                        
                        # 映射到标准情绪标签
                        from settings import EMOTION_LABEL_MAPPING
                        mapped_emotion = EMOTION_LABEL_MAPPING.get(emotion, emotion)
                        print(f"映射后情绪标签: {mapped_emotion}")
                        ...
                        if emotion_confidence >= EMOTION_DISPLAY_THRESHOLD:
                            print(f"✅ 情绪置信度 {emotion_confidence:.4f} 达到显示阈值 {EMOTION_DISPLAY_THRESHOLD}")
                            return mapped_emotion, emotion_confidence
                        else:
                            print(f"❌ 情绪置信度 {emotion_confidence:.4f} 未达到显示阈值 {EMOTION_DISPLAY_THRESHOLD}，返回unknown")
                            return "unknown", emotion_confidence
                    else:
                        print(f"❌ 类别ID超出范围: {class_id} >= {len(emotion_labels)}")
                else:
                    print("❌ 没有类别信息")
            else:
                print("❌ 没有检测框")
                ...
                if hasattr(result, 'names') and result.names:
                    print(f"名称映射: {result.names}")
                    # 如果有检测结果但没有框，可能是分类模式
                    if len(result.names) > 0:
                        emotion = list(result.names.values())[0]
                        print(f"从names获取的情绪: {emotion}")
                        ...
                        # 检查默认置信度是否达到显示阈值
                        if EMOTION_DEFAULT_CONFIDENCE >= EMOTION_DISPLAY_THRESHOLD:
                            print(f"✅ 默认置信度 {EMOTION_DEFAULT_CONFIDENCE} 达到显示阈值 {EMOTION_DISPLAY_THRESHOLD}")
                            return mapped_emotion, EMOTION_DEFAULT_CONFIDENCE
                        else:
                            print(f"❌ 默认置信度 {EMOTION_DEFAULT_CONFIDENCE} 未达到显示阈值 {EMOTION_DISPLAY_THRESHOLD}，返回unknown")
                            return "unknown", EMOTION_DEFAULT_CONFIDENCE
        
        print("❌ 没有有效结果，返回默认值")
        return "unknown", NEUTRAL_EMOTION_CONFIDENCE
...
def recognize_action(frame, action_model):
    """识别行为"""
    try:
        print(f"=== 行为识别开始 ===")
        print(f"输入图像形状: {frame.shape}")
        ...
        if len(results) > 0:
            ...
            if hasattr(result, 'boxes') and result.boxes is not None and len(result.boxes) > 0:
                ...
                if hasattr(result.boxes, 'cls') and result.boxes.cls is not None:
                    ...
                    # 根据类别ID获取行为标签
                    if class_id < len(action_labels):
                        action = action_labels[class_id]
                        print(f"原始行为标签: {action}")
                        
                        # 映射到中文行为标签
                        from settings import ACTION_LABEL_MAPPING
                        mapped_action = ACTION_LABEL_MAPPING.get(action, action)
                        print(f"映射后行为标签: {mapped_action}")
                        
                        # 使用检测置信度作为行为置信度
                        action_confidence = confidence
                        
                        # 检查是否达到显示阈值
                        if action_confidence >= ACTION_DISPLAY_THRESHOLD:
                            print(f"✅ 行为置信度 {action_confidence:.4f} 达到显示阈值 {ACTION_DISPLAY_THRESHOLD}")
                            return mapped_action, action_confidence
                        else:
                            print(f"❌ 行为置信度 {action_confidence:.4f} 未达到显示阈值 {ACTION_DISPLAY_THRESHOLD}，返回unknown")
                            return "unknown", action_confidence
                    else:
                        print(f"❌ 类别ID超出范围: {class_id} >= {len(action_labels)}")
                else:
                    print("❌ 没有类别信息")
            else:
                print("❌ 没有检测框")
                ...
                if hasattr(result, 'names') and result.names:
                    ...
                    # 如果有检测结果但没有框，可能是分类模式
                    if len(result.names) > 0:
                        action = list(result.names.values())[0]
                        print(f"从names获取的行为: {action}")
                        ...
                        # 尝试获取分类置信度
                        action_confidence = 0.0 # 默认值
                        if hasattr(result, 'probs') and result.probs is not None and hasattr(result.probs, 'data'):
                            try:
                                class_id_from_names = list(result.names.keys()).index(action)
                                if class_id_from_names < len(action_labels) and class_id_from_names < len(result.probs.data):
                                    action_confidence = float(result.probs.data[class_id_from_names])
                                    print(f"从probs获取的分类置信度: {action_confidence:.4f}")
                                else:
                                    print(f"❌ 类别ID或probs索引超出范围: {class_id_from_names} (len(action_labels): {len(action_labels)}, len(probs.data): {len(result.probs.data)})")
                                    return "unknown", 0.0 # 如果索引超出范围，则认为无法识别
                            except ValueError:
                                print(f"❌ 无法从names中找到行为标签 '{action}' 对应的class_id")
                                return "unknown", 0.0 # 如果无法找到对应的class_id，则认为无法识别
                        else:
                            print("⚠️ 模型在分类模式下未提供probs属性，无法确定行为置信度，返回unknown")
                            return "unknown", 0.0 # 如果没有probs，直接返回unknown，不显示
...
def process_frame_emotion_only(frame, face_model, emotion_model, face_recognition_model=None, face_database=None):
    """仅处理表情识别 - 先进行人脸检测，只有是人脸的才进行情绪识别和人脸识别"""
    print(f"\n=== 表情识别处理开始 ===")
    print(f"输入图像尺寸: {frame.shape}")
    ...
    for result in face_results:
        ...
        if boxes is not None and len(boxes) > 0:
            for box in boxes:
                ...
                print(f"检测到目标 - 置信度: {confidence:.4f}, 边界框: [{x1}, {y1}, {x2}, {y2}]")
                ...
                if confidence > FACE_DETECTION_THRESHOLD:
                    ...
                    # 第二步：检测关键点并进行人脸对齐
                    print("第二步：检测关键点并进行人脸对齐...")
                    landmarks = detect_landmarks(frame, [x1, y1, x2, y2])
                    if landmarks is not None and len(landmarks) >= 5:
                        aligned_face = align_face(face_crop, landmarks)
                        print(f"人脸对齐后尺寸: {aligned_face.shape}")
                        print(f"检测到关键点数量: {len(landmarks)}")
                    else:
                        print("⚠️ 关键点检测失败，使用原始裁剪")
                        aligned_face = cv2.resize(face_crop, (112, 112))
                    
                    # 第三步：进行人脸识别（如果提供了模型和数据库）
                    identity, similarity = "Unknown", 0.0
                    if face_recognition_model is not None and face_database is not None:
                        print("第三步：进行人脸识别...")
                        features = extract_features(aligned_face, face_recognition_model, "emotion_query")
                        if features is not None:
                            identity, similarity = match_face(features, face_database)
                            if identity is None:
                                identity = "Unknown"
                        print(f"人脸识别结果: {identity}, 相似度: {similarity:.4f}")
                    else:
                        print("⚠️ 未提供人脸识别模型或数据库，跳过人脸识别")
                    
                    # 第四步：进行情绪识别
                    print("第四步：进行情绪识别...")
                    emotion, emotion_confidence = recognize_emotion(aligned_face, emotion_model)
                    
                    print(f"情绪识别结果: {emotion}, 置信度: {emotion_confidence:.4f}")
                    ...
                    print(f"❌ 不是人脸，置信度 {confidence:.4f} 低于阈值 {FACE_DETECTION_THRESHOLD}，跳过情绪识别")
        else:
            print("❌ 没有检测到任何目标")
    
    print(f"处理完成，检测到 {len(faces)} 个有效人脸")
    return faces

def process_frame_action_analysis(frame, face_model, face_recognition_model, action_model, face_database):
    """行为分析 - 人脸识别 + 行为识别"""
    print(f"\n=== 行为分析处理开始 ===")
    print(f"输入图像尺寸: {frame.shape}")
        ...
        if boxes is not None and len(boxes) > 0:
            for box in boxes:
                ...
                if confidence > FACE_DETECTION_THRESHOLD:
                    ...
                    # 生成唯一的查询ID
                    query_id = f"action_query_{int(time.time())}"
                    
                    # 第三步：检测关键点并进行人脸对齐
                    print("第三步：检测关键点并进行人脸对齐...")
                    landmarks = detect_landmarks_direct(face_crop)
                    ...
                    # 第五步：行为识别
                    print("第五步：进行行为识别...")
                    action, action_confidence = recognize_action(frame, action_model)
                    ...
                    print(f"❌ 不是人脸，置信度 {confidence:.4f} 低于阈值 {FACE_DETECTION_THRESHOLD}，跳过识别")
        else:
            print("❌ 没有检测到任何目标")
...
def process_frame_combined(frame, face_model, face_recognition_model, emotion_model, face_database):
    """综合识别 - 人脸识别 + 表情识别 - 先进行人脸检测，只有是人脸的才进行识别"""
    print(f"\n=== 综合识别处理开始 ===")
    print(f"输入图像尺寸: {frame.shape}")
...
async def emotion_recognize_image(file: UploadFile = File(...)):
    ...
    try:
        ...
        # 处理图片
        faces = process_frame_emotion_only(image, face_model, emotion_model, face_recognition_model, face_database)
...
async def emotion_recognize_video(file: UploadFile = File(...)):
    ...
    try:
        ...
        while True:
            ...
            if frame_count % VIDEO_FRAME_INTERVAL == 0:  # 使用配置的帧间隔
                faces = process_frame_emotion_only(frame, face_model, emotion_model, face_recognition_model, face_database)
                if faces:
                    all_faces.append({"frame": frame_count, "faces": faces})
...
# 行为分析接口组
@app.post("/action/analyze_image")
async def action_analyze_image(file: UploadFile = File(...)):
    """行为分析 - 图片"""
    try:
        # 读取图片
        image_data = await file.read()
        ...
        # 处理图片
        faces = process_frame_action_analysis(image, face_model, face_recognition_model, action_model, face_database)
...
@app.post("/action/analyze_video")
async def action_analyze_video(file: UploadFile = File(...)):
    """行为分析 - 视频"""
    try:
        # 保存临时文件
        temp_filename = f"temp_action_video_{int(time.time())}.mp4"
        with open(temp_filename, "wb") as f:
            f.write(await file.read())
        ...
        while True:
            ...
            if frame_count % VIDEO_FRAME_INTERVAL == 0:  # 使用配置的帧间隔
                faces = process_frame_action_analysis(frame, face_model, face_recognition_model, action_model, face_database)
                if faces:
                    all_faces.append({"frame": frame_count, "faces": faces})
...
async def combined_recognize_stream(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            stream_type = config.get("type")  # "rtsp" 或 "camera" 或 "frontend_frame"
            print(f"后端接收到 stream_type: {stream_type}") # 添加调试日志
            ...
            if stream_type == "rtsp":
                ...
                cap = cv2.VideoCapture(int(camera_id))
            elif stream_type == "frontend_frame": # 新增：处理前端发送的帧
                while True:
                    try:
                        frame_data_b64 = await websocket.receive_text() # 接收Base64编码的帧
                        # 解码Base64字符串为OpenCV图像帧
                        encoded_data = frame_data_b64.split(',')[1] # 移除data:image/jpeg;base64,
                        np_arr = np.frombuffer(base64.b64decode(encoded_data), np.uint8)
                        frame = cv2.imdecode(np_arr, cv2.IMREAD_COLOR)

                        if frame is None:
                            print("警告：无法解码前端发送的图像帧")
                            continue
                        ...
                        await websocket.send_text(json.dumps(result))
                        await asyncio.sleep(STREAM_PROCESSING_INTERVAL / 1000)

                    except WebSocketDisconnect:
                        print("WebSocket连接断开")
                        break
                    except Exception as e:
                        print(f"处理前端帧失败: {str(e)}")
                        await websocket.send_text(json.dumps({"error": f"处理前端帧失败: {str(e)}"}))
                        # continue # 遇到错误时继续接收后续帧
                break # 跳出外层while True，因为内层循环已经处理了WebSocketDisconnect

            else:
                await websocket.send_text(json.dumps({"error": "不支持的流类型或帧源"}))
                continue
            
            if stream_type != "frontend_frame" and not cap.isOpened(): # 只有非前端帧模式才需要检查cap.isOpened()
                await websocket.send_text(json.dumps({"error": "无法打开流"}))
                continue
            
            frame_count = 0
            if stream_type != "frontend_frame": # 只有非前端帧模式才执行视频读取循环
                while True:
                    ret, frame = cap.read()
                    if not ret:
                        break
...
# 实际项目中，这里应该替换为数据库操作
users_db = {
    "admin": {"password_hash": bcrypt.hashpw("123456".encode('utf-8'), bcrypt.gensalt()).decode('utf-8'), "id": "user_admin", "role": "admin"}
}

# 简化的TOKEN存储，实际项目应使用JWT等
tokens_db = {}
...
async def register_user(user: dict):
    ...
    if not username or not password:
        return JSONResponse(status_code=400, content={"code": 1001, "msg": "用户名和密码不能为空"})

    if username in users_db:
        return JSONResponse(status_code=400, content={"code": 1002, "msg": "用户已存在"})

    hashed_password = bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
    user_id = str(uuid.uuid4())
    users_db[username] = {"password_hash": hashed_password, "id": user_id, "role": "user"}
    print(f"用户 {username} 注册成功")
    return {"code": 0000, "msg": "注册成功", "data": {"userId": user_id, "username": username}}
...
async def login_user(user: dict):
    ...
    if not stored_user or not bcrypt.checkpw(password.encode('utf-8'), stored_user["password_hash"].encode('utf-8')):
        return JSONResponse(status_code=401, content={"code": 2001, "msg": "用户名或密码错误"})

    # 简化TOKEN生成
    access_token = str(uuid.uuid4())
    refresh_token = str(uuid.uuid4())
    tokens_db[access_token] = {"username": username, "refresh_token": refresh_token}
    tokens_db[refresh_token] = {"username": username, "access_token": access_token}

    print(f"用户 {username} 登录成功")
    return {"code": 0000, "msg": "登录成功", "data": {"token": access_token, "refreshToken": refresh_token, "userId": stored_user["id"]}}

@app.get("/user/getUserInfo")
async def get_user_info(Authorization: str = Header(None)):
    """获取用户信息"""
    if not Authorization or not Authorization.startswith("Bearer "):
        return JSONResponse(status_code=401, content={"code": 1002, "msg": "缺少或无效的TOKEN"})
    token = Authorization.split(" ")[1]

    token_info = tokens_db.get(token)
    if not token_info or "username" not in token_info:
        return JSONResponse(status_code=401, content={"code": 1002, "msg": "TOKEN已过期或无效"})
    
    username = token_info["username"]
    user_data = users_db.get(username)

    if not user_data:
        return JSONResponse(status_code=404, content={"code": 2002, "msg": "用户信息未找到"})

    print(f"获取用户 {username} 信息成功")
    return {"code": 0000, "msg": "获取成功", "data": {"userId": user_data["id"], "nickname": username, "roles": [user_data["role"]], "buttons": []}}

@app.post("/user/refreshToken")
async def refresh_token(token_data: dict):
    """刷新TOKEN"""
    refresh_token = token_data.get("refresh_token")
    if not refresh_token:
        return JSONResponse(status_code=400, content={"code": 1001, "msg": "缺少 refresh_token"})

    token_info = tokens_db.get(refresh_token)
    if not token_info or "username" not in token_info or "access_token" not in token_info:
        return JSONResponse(status_code=401, content={"code": 1002, "msg": "refresh_token 无效或已过期"})
    
    username = token_info["username"]

    # 清除旧的TOKEN
    old_access_token = token_info["access_token"]
    if old_access_token in tokens_db: # 检查是否存在旧的 access_token
        del tokens_db[old_access_token]
    if refresh_token in tokens_db: # 清除旧的 refresh_token
        del tokens_db[refresh_token]

    # 生成新的TOKEN
    new_access_token = str(uuid.uuid4())
    new_refresh_token = str(uuid.uuid4())
    tokens_db[new_access_token] = {"username": username, "refresh_token": new_refresh_token}
    tokens_db[new_refresh_token] = {"username": username, "access_token": new_access_token}

    print(f"用户 {username} 刷新TOKEN成功")
    return {"code": 0000, "msg": "TOKEN刷新成功", "data": {"token": new_access_token, "refreshToken": new_refresh_token}}
